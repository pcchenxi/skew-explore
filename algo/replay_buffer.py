...
class HindsightExperienceReplayWrapper(object):
    ...
    def add(self, obs_t, action, reward, obs_tp1, done):
        ...
        obs_dict = self.env.convert_obs_to_dict(obs_t)
        self.episode_obs_trajectory.append(obs_dict['observation'])
        if done:
            ...
            obs_dict = self.env.convert_obs_to_dict(obs_tp1)
            self.episode_obs_trajectory.append(obs_dict['observation'])
    ...
    def _sample_achieved_goals(self, episode_transitions, transition_idx):
        ...
        :return: (np.ndarray) an achieved goal
        """
        # additional 30% of the goal is sampled from the current goal proposing model
        rig_goal_percentage = 0.3 #0.7 #0.3
        rig_goal_num = int(self.n_sampled_goal * rig_goal_percentage) + 1

        current_goals = self.env.env.unwrapped.goals 
        rig_goals_index = np.random.randint(len(current_goals), size=rig_goal_num)
        rig_goals = current_goals[rig_goals_index]

        her_goals = [self._sample_achieved_goal(episode_transitions, transition_idx) for _ in range(self.n_sampled_goal)]

        goals = np.concatenate((rig_goals, her_goals))
        return goals
    ...
    def _store_episode(self):
        ...
        for transition_idx, transition in enumerate(self.episode_transitions):
            ...
            # obs_next_dict = self.env.convert_obs_to_dict(obs_tp1)
            # reward = self.env.compute_reward(obs_next_dict['achieved_goal'], obs_next_dict['desired_goal'], np.array(self.episode_obs_trajectory), None)
            ...
            if self.n_sampled_goal != 0:
                ...
                for goal in sampled_goals:
                    ...
                    # reward = self.env.compute_reward(next_obs_dict['achieved_goal'], goal, np.array(self.episode_obs_trajectory), None)
                    # reward is computed using trajectory instead of single state
                    reward = self.env.compute_reward(next_obs_dict['achieved_goal'], goal, np.array(self.episode_obs_trajectory)[:transition_idx+1], None)
                    done = False
